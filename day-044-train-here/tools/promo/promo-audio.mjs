/* プロモ動画の BGM。外部パッケージも外部音源も使わず、固定シードの純JSで合成する。
   ...
   既定（--variant a）は必ず同じフォルダへ promo-audio.wav を 30.000秒 / 48kHz / 16bit / stereo で書く。
...
const BAR = BEAT * 4;               // 2.6087秒（30秒 ≈ 11.5小節）
...
/* 音色・音列・合成処理は Day 033 から続くもの。鍵の名前は据え置き、指す場面だけ今日のものに合わせる。 */
const SECTION = {
  answer: onBeat(sceneStart('S1')),   // 都心の全景が開ける
  coins: onBeat(sceneStart('S2')),    // 一覧から1編成を選ぶ
  near: onBeat(sceneStart('S3')),     // 編成にカメラが追従する
  chart: onBeat(sceneStart('S4')),    // 前方の風景
  confirm: onBeat(T_CONFIRM_TAP),     // 視点が線路の高さへ入れ替わる（いちばんの山）
  facts: onBeat(sceneStart('S5')),    // 新宿のあたりへ寄る
  caveat: onBeat(sceneStart('S6')),   // 架空の運行デモであることの明示
...
for (let bar = 0; bar < BAR_COUNT; bar += 1) {
  ...
  // ベース：1拍目と3拍目にルート
  if (barStart >= SECTION.answer && barStart < SECTION.caveat) {
    // 低域（<120Hz）が全体と1.6dBしか差が無かったため、ベースとキックをまとめて -10dB強 絞った
    const level = barStart < SECTION.coins ? .135 : .23;
  ...
  // ドラム
  if (barStart >= SECTION.answer && barStart < SECTION.caveat) {
    const intro = barStart < SECTION.coins;                 // S1 はキックとハットだけ静かに
    ...
    for (const at of kicks) {
      const time = beat(at);
      if (time >= SECTION.caveat) continue;
      kickTimes.push(time);
  ...
  const arpDensity = barStart < SECTION.answer ? .62
    : barStart < SECTION.coins ? .7
      : barStart < SECTION.caveat ? .8
  ...
  for (let step = 0; step < 8 && arpDensity > 0; step += 1) {
    ...
    addPiano({
      ...
      gain: (VARIANT === 'a' ? .085 : .1) * velocity
        * (barStart < SECTION.answer ? 1.2 : barStart >= SECTION.caveat ? .75 : 1),
      pan: (random() * 2 - 1) * .42
    });
  }

  // 主旋律：2小節ごと。S3〜S4 ではオクターブ上の副旋律を足して持ち上げる
  const leadOn = bar % 2 === 0 && barStart >= SECTION.coins && barStart < SECTION.caveat;
  if (leadOn) {
    const motif = bar % 4 === 0 ? MOTIF_A : MOTIF_B;
    const doubled = barStart >= SECTION.near && barStart < SECTION.facts;
    for (const [offset, midi, length] of motif) {
      const time = beat(offset);
      if (time >= SECTION.caveat) continue;
      addPiano({
        start: time, duration: BEAT * length + .35, midi,
        gain: (VARIANT === 'a' ? .13 : .15) * (barStart >= SECTION.facts ? .8 : 1), pan: -.1
      });
      if (doubled) {
        addPiano({
...
// 場面の変わり目に置く、ごく薄い持ち上げ
for (const at of [SECTION.coins, SECTION.near, SECTION.facts]) {
