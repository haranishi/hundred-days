/* 1コマ＝時刻 t の純関数。__seek(t) を呼ぶと、その時刻の画がそのまま出る。
   ...
   アプリは reducedMotion で撮るので、アプリ自身のアニメーション（見出しの appear）は動かない。
...
import {
  ...
  PROMISE_START, TAPS, TITLE_START, HOOK_ENTRANCE_START, TABLE_START,
  appSceneAt, backdropDrift, curveRevealAt, captionAt, clamp, easeInOutCubic, easeOutCubic, mix,
...
const PLANS = {
  hook: { save: savedPlace, steps: [] },
  answer: { save: savedPlace, steps: [] },
  curve: { save: savedPlace, steps: [] },
  'pick-start': { save: null, steps: [] },
  'pick-dialog': { save: null, steps: ['pick'] },
  'pick-pref': { save: null, steps: ['pick', 'pref'] },
  'pick-station': { save: null, steps: ['pick', 'pref', 'station'] },
  'pick-done': { save: null, steps: ['pick', 'pref', 'station', 'confirm'] },
  range: { save: savedPlace, steps: [] },
  tables: { save: savedPlace, steps: [] }
};
...
const STORAGE_NAME = 'day-034-tide-now';

async function runStep(step) {
  const doc = appDocument();
  if (step === 'pick') {
    doc.getElementById('pick-station').click();
    await waitUntil(() => doc.getElementById('station-dialog').open, 20_000, '場所選びが開きませんでした');
    return;
  }
  if (step === 'pref' || step === 'station') {
    const select = doc.getElementById(step === 'pref' ? 'pref-select' : 'station-select');
    const value = step === 'pref' ? '東京都' : PLACE_CODE;
  ...
  if (step === 'confirm') {
    ...
    const rect = doc.getElementById('station-confirm').getBoundingClientRect();
    state.targets.set('confirm', { left: rect.left, top: rect.top, width: rect.width, height: rect.height });
    doc.getElementById('station-confirm').click();
    await waitForPlace();
    return;
  }
...
const waitForPlace = () => waitUntil(() => {
  const doc = appDocument();
  const expected = {
    'station-name': PLACE_LABEL,
    answer: 'いま、満ち潮',
    'answer-sub': '次の満潮は 16:57（201cm）',
    remaining: 'あと1時間57分',
    'tide-name': '大潮',
    'now-level': 'いまの潮位 およそ165cm',
    range: 'きょうの干満差 171cm（東京では大きい方）',
    'as-of': '15:00 現在'
  };
  return appState() === 'ready'
    && doc.getElementById('app').dataset.place === 'picked'
    && doc.getElementById('answer').dataset.kind === 'rising'
    && Object.entries(expected).every(([id, text]) => doc.getElementById(id).textContent === text)
    && doc.querySelectorAll('#curve .event-dot').length === 4
    && doc.querySelectorAll('#events-today tbody tr').length === 4
    && doc.querySelectorAll('#events-tomorrow tbody tr').length === 4;
}, 20_000, '固定した東京の潮位・表が出ませんでした');
...
function scrollFor(t) {
  ...
  const range = Math.max(0, topOf(doc.getElementById('range')) - headline.offsetHeight - SCROLL_MARGIN);
  let origin = answer;
  let target = answer;
  if (view.target === 'curve') {
    const curve = doc.getElementById('curve');
    target = topOf(curve) - (win.innerHeight - curve.getBoundingClientRect().height) / 2;
  } else if (view.target === 'place') {
    target = topOf(doc.querySelector('.place-row')) - 180;
  } else if (view.target === 'range') {
    // #range を基準に、すぐ上の見出し内にある潮名バッジも残す。
    target = range;
  } else if (view.target === 'tables') {
    origin = range;
    target = topOf(doc.getElementById('events-today')) - SCROLL_MARGIN;
  }
  const max = Math.max(0, doc.documentElement.scrollHeight - win.innerHeight);
  return mix(clamp(origin, 0, max), clamp(target, 0, max), view.amount);
}

function overrideApp(t) {
  const doc = appDocument();
  doc.documentElement.style.scrollBehavior = 'auto';
  doc.body.style.paddingBottom = t >= TABLE_START ? `${appWindow().innerHeight}px` : '';
  const answer = doc.getElementById('answer');
  answer.style.animation = 'none';
  setEntrance(answer, t, t < TITLE_START ? HOOK_ENTRANCE_START : ANSWER_START, .6, { y: 16, scaleFrom: 1 });
  const curve = doc.getElementById('curve');
  const line = curve.querySelector('.water-line');
  if (line) {
    const reveal = curveRevealAt(t);
    const length = line.getTotalLength();
    line.style.strokeDasharray = `${length} ${length}`;
    line.style.strokeDashoffset = String(length * (1 - reveal.line));
    // 面の塗りが未描画の線を先に見せないよう、線と同じ進捗でクリップする。
    const area = curve.querySelector('path[fill]');
    if (area) area.style.clipPath = `inset(0 ${(1 - reveal.line) * 100}% 0 0)`;
    // SVGのDOM順は満潮2点→干潮2点なので、横座標で時刻順に並べる。
    [...curve.querySelectorAll('.event-dot')]
      .sort((a, b) => Number(a.getAttribute('cx')) - Number(b.getAttribute('cx')))
      .forEach((dot, index) => {
        const opacity = String(curveRevealAt(t, index).event);
        dot.style.opacity = opacity;
        if (dot.nextElementSibling?.tagName.toLowerCase() === 'text') dot.nextElementSibling.style.opacity = opacity;
      });
    const now = t < TITLE_START
      ? progress(t, HOOK_ENTRANCE_START, HOOK_ENTRANCE_START + .6, easeOutCubic) : reveal.now;
    for (const node of curve.querySelectorAll('.now-line, .now-dot, .now-label, .now-level-label')) node.style.opacity = String(now);
    const nowLine = curve.querySelector('.now-line');
    nowLine.style.transformBox = 'fill-box';
    nowLine.style.transformOrigin = 'center bottom';
    nowLine.style.transform = `scaleY(${now})`;
  }
...
function targetBox(name) {
  const selectors = { pick: '#pick-station', confirm: '#station-confirm',
    pref: '#pref-select', station: '#station-select' };
