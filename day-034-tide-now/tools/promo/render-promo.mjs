...
const work = mkdtempSync(join(tmpdir(), 'day034-promo-'));
...
try {
  ...
  // 年・地点ごとにデモと同じ固定応答を返す。無いfixtureは404。
  await context.route('**://www.data.jma.go.jp/kaiyou/data/db/tide/suisan/txt/*/*.txt', async (route) => {
    const parts = new URL(route.request().url()).pathname.split('/');
    const year = parts.at(-2), code = parts.at(-1).slice(0, -4);
    if (!/^[A-Z][A-Z0-9]$/.test(code) || !/^\d{4}$/.test(year)) {
  ...
  const known = new Set([`127.0.0.1:${server.address().port}`, 'www.data.jma.go.jp']);
  ...
  if (preview) {
    ...
    execFileSync('ffmpeg', ['-y','-loglevel','error','-framerate','1','-i',join(work,'frame_%05d.jpg'),
      '-vf',`scale=360:640:flags=lanczos,tile=4x${rows}:padding=8:margin=8:color=f4f7f9`,'-frames:v','1','-q:v','2',contactPath], { stdio:['ignore','ignore','inherit'] });
  ...
  console.log(`想定外の外部通信: ${strangers.size ? [...strangers].join(', ') : 'なし（潮位表は固定応答）'}`);
} catch (error) {
