...
/* 単独ファイルを持つ2次メッシュと、持たない側（親の4桁）が1つの円に同居する場合。
   4桁のファイルには単独ファイルぶんが入っていないので、両方取らないと片側を丸ごと落とす。
   実データで確かめた地点＝那覇の東（親 3927 から72件、2次 392725 から63件） */
test('2次の単独ファイルと親の4桁が同じ円に入ったら、両方を取る', () => {
  const codes = filesForCircle(real, { lat: 26.20833, lng: 127.74975 }, 1000);
  assert.deepEqual(codes, ['3927', '392725']);
  assert.ok(real.files['392725'] >= 2000, '392725 は単独ファイルを持つ側');
  assert.equal(real.files['392726'], undefined, '東隣は単独ファイルを持たず親へまとまっている');
});
