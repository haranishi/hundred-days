...
async function begin(page) {
  ...
  await page.waitForFunction(() => window.__day046?.state() === 'ready', null, { timeout: 20000 });
...
// 通しで光速まで約91秒かかる。録画は等速で撮り、仕上げで5倍に早回しして18秒台にする。
export default async function demo(page, h) {
  await begin(page);
  await page.waitForFunction(() => window.__day046.state() === 'result', null, { timeout: 130000 });
  await page.mouse.up();
  await h.pause(2500);
}
...
export async function shotSetup(page) {
  await begin(page);
  await page.waitForFunction(() => window.__day046.snapshot().speed >= 120000, null, { timeout: 90000 });
  await page.mouse.up();
}
