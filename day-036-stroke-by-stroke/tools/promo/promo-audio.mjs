...
const SECTION = {
  ...
  slow: onBeat(sceneStart('S2')),
  brush: onBeat(sceneStart('S3')),
  guide: onBeat(sceneStart('S4')),
  confirm: onBeat(T_CONFIRM_TAP),
  save: onBeat(sceneStart('S5')),
...
for (let bar = 0; bar < BAR_COUNT; bar += 1) {
  ...
  if (barStart < SECTION.end) {
    ...
    for (const [voice, midi] of chord.pad.entries()) {
      addPad({
        ...
        gain: (bar === 0 ? .26 : .22) * (barStart < SECTION.slow ? .96 : 1) * hookLift * (voice === 0 ? .40 : 1),
  ...
  if (barStart >= SECTION.title && barStart < SECTION.promise) {
    ...
    const level = barStart < SECTION.slow ? .135 : .23;
  ...
  // ドラム
  if (barStart >= SECTION.title && barStart < SECTION.promise) {
    const intro = barStart < SECTION.slow;                 // S1 はキックとハットだけ静かに
  ...
  const leadOn = bar % 2 === 0 && barStart >= SECTION.slow && barStart < SECTION.promise;
  if (leadOn) {
    const motif = bar % 4 === 0 ? MOTIF_A : MOTIF_B;
    const doubled = barStart >= SECTION.brush && barStart < SECTION.save;
    for (const [offset, midi, length] of motif) {
      const time = beat(offset);
      if (time >= SECTION.promise) continue;
      addPiano({
        start: time, duration: BEAT * length + .35, midi,
        gain: (VARIANT === 'a' ? .13 : .15) * (barStart >= SECTION.save ? .8 : 1), pan: -.1
      });
      if (doubled) {
        addPiano({
...
// 場面の変わり目に置く、ごく薄い持ち上げ
for (const at of [SECTION.slow, SECTION.brush, SECTION.save]) {
