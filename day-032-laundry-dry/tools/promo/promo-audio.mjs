/* プロモ動画の BGM。外部パッケージも外部音源も使わず、固定シードの純JSで合成する。
   ...
   render-promo.mjs は wav が無ければこのスクリプトを「引数なしで」実行するので、
   既定（--variant a）は必ず同じフォルダへ promo-audio.wav を 36.000秒 / 48kHz / 16bit / stereo で書く。
...
import { DURATION_SECONDS, RESULT_START, STORYBOARD, TAPS, T_THICK_TAP } from './timeline.mjs';
...
/* 名前は Day 031 から引き継いだまま（曲の役割を表している）。中身はこの Day の絵コンテに合わせた。
   S0 フック／S1 タイトル／S2 場所／S3 乾く時刻／S4 厚手と日かげ／S5 帯／S6 根拠／S7 約束／S8 エンド */
const SECTION = {
  title: onBeat(sceneStart('S1')),      // ドラムが静かに入る
  pref: onBeat(sceneStart('S2')),       // フルのグルーヴ
  town: onBeat(sceneStart('S3')),
  hint: onBeat(T_THICK_TAP),            // ここでローパスがわずかに開く
  wiki: onBeat(sceneStart('S5')),       // 帯が伸びる見せ場。副旋律を足して持ち上げる
  result: onBeat(sceneStart('S6')),     // 根拠を開くところ。いちばん静かに落とす
  promise: onBeat(sceneStart('S7')),    // ドラムを抜く
  end: onBeat(sceneStart('S8'))         // 終止和音
};
...
const keys = makeBus();     // 鍵盤（アルペジオ＋主旋律）
const bass = makeBus();
...
function addElectricPiano({ start, duration, midi, gain, pan, index0 = 2.7, tauMod = .12, tauAmp = .85 }) {
  ...
  for (let index = first; index < last; index += 1) {
    ...
    spread(keys, index, sample, leftGain, rightGain);
  }
}
...
const addKeys = (options) => (VARIANT === 'a' ? addElectricPiano(options) : addPluckedString(options));
...
for (let bar = 0; bar < BAR_COUNT; bar += 1) {
  ...
  if (barStart < SECTION.end) {
    // S0（ドラム前）は正解チャイムの増量だけで S2 との差が6dB以内に収まったため、パッド自体はほぼ据え置き
    ...
    for (const [voice, midi] of chord.pad.entries()) {
      addPad({
        ...
        gain: (bar === 0 ? .26 : .22) * (barStart < SECTION.pref ? .96 : 1) * hookLift * (voice === 0 ? .40 : 1),
  ...
  if (barStart >= SECTION.title && barStart < SECTION.promise) {
    ...
    const level = barStart < SECTION.pref ? .135 : .23;
  ...
  // ドラム
  if (barStart >= SECTION.title && barStart < SECTION.promise) {
    const intro = barStart < SECTION.pref;                 // S1 はキックとハットだけ静かに
  ...
  for (let step = 0; step < 8 && arpDensity > 0; step += 1) {
    ...
    addKeys({
      ...
      // S0 は正解チャイムの増量だけで S2 との差が縮まったので、ここはわずかな底上げに留める
  ...
  // 主旋律：2小節ごと。S4 ではオクターブ上の副旋律を足して持ち上げる
  const leadOn = bar % 2 === 0 && barStart >= SECTION.pref && barStart < SECTION.promise;
  if (leadOn) {
    const motif = bar % 4 === 0 ? MOTIF_A : MOTIF_B;
    const doubled = barStart >= SECTION.wiki && barStart < SECTION.result;
    for (const [offset, midi, length] of motif) {
      const time = beat(offset);
      if (time >= SECTION.promise) continue;
      addKeys({
        start: time, duration: BEAT * length + .35, midi,
        gain: (VARIANT === 'a' ? .13 : .15) * (barStart >= SECTION.result ? .8 : 1), pan: -.1
      });
      if (doubled) {
        addKeys({
...
// 場面の変わり目に置く、ごく薄い持ち上げ
for (const at of [SECTION.pref, SECTION.wiki, SECTION.result]) {
...
for (const [index, tap] of TAPS.entries()) {
  ...
  /* 正解チャイムの強さは場面の賑やかさに合わせる。TAPS の並びは
     0=フック（S0、無音に近い）1=都道府県（S2）2=ヒント 3=市町村（S3・フィルの小節で一番賑やか）。
     一律の音量だと S0 では効きすぎて場面の弧を壊し、S3 では負けて聞き取れなくなる。 */
  const chimeBoost = index === 0 ? .55 : index === 3 ? 1.3 : 1;
...
{
  const boostAt = (time) => {
    if (time < SECTION.hint - .4 || time > SECTION.hint + 2.2) return 1;
    return time < SECTION.hint
      ? 1 + .55 * (time - (SECTION.hint - .4)) / .4
      : 1 + .55 * Math.max(0, 1 - (time - SECTION.hint) / 2.2);
  };
  let left = 0;
  let right = 0;
...
/* 鍵盤に付点8分のピンポンディレイを薄く。入力は右へ、以後 L↔R で受け渡す。 */
{
  ...
  for (let index = 0; index < FRAME_COUNT; index += 1) {
    ...
    const mono = (keys.l[index] + keys.r[index]) * .5;
    lineR[index + delay] += mono * .6 + dampL * feedback;
    lineL[index + delay] += dampR * feedback;
    keys.l[index] += tapL * wet;
    keys.r[index] += tapR * wet;
  }
}
...
const reverb = (() => {
  ...
  for (let index = 0; index < FRAME_COUNT; index += 1) {
    send.l[index] = pad.l[index] * .22 + keys.l[index] * .26 + snare.l[index] * .3 + sfx.l[index] * .22;
    send.r[index] = pad.r[index] * .22 + keys.r[index] * .26 + snare.r[index] * .3 + sfx.r[index] * .22;
  }
...
for (let index = 0; index < FRAME_COUNT; index += 1) {
  ...
  left[index] = (pad.l[index] + keys.l[index] + reverb.l[index] * wet) * ducked * tapDucked
    + (bass.l[index] + drums.l[index] + snare.l[index]) * tapDucked + sfx.l[index];
  right[index] = (pad.r[index] + keys.r[index] + reverb.r[index] * wet) * ducked * tapDucked
...
for (let index = 0; index < FRAME_COUNT; index += 1) {
  ...
  const master = Math.sin(Math.PI * .5 * fadeIn) * Math.sin(Math.PI * .5 * Math.max(0, fadeOut));
