...
import { DEFAULT_FPS, DURATION_SECONDS, previewTimes } from './timeline.mjs';
...
const work = mkdtempSync(join(tmpdir(), 'day030-promo-'));
let server;
let browser;
try {
  ...
  // アプリ初期化より前に E2E の Map フックと長周期タイマー抑止を iframe にも入れる。
  await context.addInitScript(() => {
    globalThis.__E2E__ = true;
    const nativeSetInterval = globalThis.setInterval.bind(globalThis);
    globalThis.setInterval = (handler, delay = 0, ...intervalArgs) => (
      Number(delay) >= 30_000 ? 0 : nativeSetInterval(handler, delay, ...intervalArgs)
    );
  });
  ...
  if (preview) {
    ...
    execFileSync('ffmpeg', ['-y','-loglevel','error','-framerate','1','-i',join(work,'frame_%05d.jpg'),
      '-vf',`scale=360:640:flags=lanczos,tile=4x${rows}:padding=8:margin=8:color=07111b`,'-frames:v','1','-q:v','2',contactPath], { stdio:['ignore','ignore','inherit'] });
