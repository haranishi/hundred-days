...
リポジトリ直下からは `node day-030-world-window/tools/promo/render-promo.mjs` と実行できます。プレビューは `preview-contact.jpg`、本番は `promo.mp4`、`promo-first-frame.png`、`promo-audio.wav` を同じフォルダへ出力します。部分確認には `--from 18 --to 24`、別の出力先には絶対パスで `--out <mp4>` を指定できます。
