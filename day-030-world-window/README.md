...
```sh
node day-030-world-window/tools/fetch-osm.mjs    # Overpass から1回取得（キャッシュがあれば再取得しない）
node day-030-world-window/tools/probe-urls.mjs   # 全URLの種別を実測（ホストごとに直列・約45分）
node day-030-world-window/tools/build-data.mjs   # 同梱JSONを生成
```
