...
import { DEFAULT_FPS, DURATION_SECONDS, WIKI_SUMMARIES, previewTimes } from './timeline.mjs';
...
try {
  ...
  /* Wikipedia は実APIを叩かない。撮るたびに本文が変わると決定性が壊れるうえ、
     記事の写真は CC BY-SA なので動画に焼き込まない（thumbnail を返さない）。 */
  await context.route('https://ja.wikipedia.org/**', (route) => {
    const title = decodeURIComponent(new URL(route.request().url()).pathname.split('/').pop());
    const extract = WIKI_SUMMARIES[title];
    if (!extract) return route.fulfill({ status:404, contentType:'application/json; charset=utf-8', body:'{}' });
  ...
  // 写真の配信元は念のため遮断する（固定応答に thumbnail は無いので、本来ここへは来ない）
  await context.route('https://*.wikimedia.org/**', (route) => route.abort());
  // 差し替えたはずの先以外へ通信していないかを見張る（撮り終わりに報告する）
  const known = new Set([`127.0.0.1:${server.address().port}`, 'ja.wikipedia.org']);
  context.on('request', (request) => {
    const host = new URL(request.url()).host;
    if (!known.has(host) && !host.endsWith('.wikimedia.org')) strangers.add(host);
  });
  ...
  console.log(`想定外の外部通信: ${strangers.size ? [...strangers].join(', ') : 'なし（Wikipedia は固定応答・写真は遮断）'}`);
} catch (error) {
