...
```sh
PLAYWRIGHT=/path/to/playwright/index.js node day-035-front-page/tools/promo/render-promo.mjs
```
...
- 貼るのは自分の過去作3本（Day 034・033・032）。中継は `day-03*/meta.json` から組み立てた固定応答に差し替えるので、外へは出ない（描画後に通信したホストを出力する）
