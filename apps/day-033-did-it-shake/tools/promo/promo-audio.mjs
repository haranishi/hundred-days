...
/* 音色・音列・合成処理は Day 032 のまま。役割名を今日の場面に合わせる。 */
const SECTION = {
  title: onBeat(sceneStart('S1')),
  answer: onBeat(sceneStart('S2')),
  bar: onBeat(sceneStart('S3')),
  place: onBeat(sceneStart('S4')),
  confirm: onBeat(T_CONFIRM_TAP),
  last: onBeat(sceneStart('S5')),
...
for (let bar = 0; bar < BAR_COUNT; bar += 1) {
  ...
  if (barStart < SECTION.end) {
    ...
    for (const [voice, midi] of chord.pad.entries()) {
      addPad({
        ...
        gain: (bar === 0 ? .26 : .22) * (barStart < SECTION.answer ? .96 : 1) * hookLift * (voice === 0 ? .40 : 1),
  ...
  if (barStart >= SECTION.title && barStart < SECTION.promise) {
    ...
    const level = barStart < SECTION.answer ? .135 : .23;
  ...
  // ドラム
  if (barStart >= SECTION.title && barStart < SECTION.promise) {
    const intro = barStart < SECTION.answer;                 // S1 はキックとハットだけ静かに
  ...
  const leadOn = bar % 2 === 0 && barStart >= SECTION.answer && barStart < SECTION.promise;
  if (leadOn) {
    const motif = bar % 4 === 0 ? MOTIF_A : MOTIF_B;
    const doubled = barStart >= SECTION.bar && barStart < SECTION.last;
    for (const [offset, midi, length] of motif) {
      const time = beat(offset);
      if (time >= SECTION.promise) continue;
      addPiano({
        start: time, duration: BEAT * length + .35, midi,
        gain: (VARIANT === 'a' ? .13 : .15) * (barStart >= SECTION.last ? .8 : 1), pan: -.1
      });
      if (doubled) {
        addPiano({
...
// 場面の変わり目に置く、ごく薄い持ち上げ
for (const at of [SECTION.answer, SECTION.bar, SECTION.last]) {
  addRiser({ start: at - .55, duration: .55, gain: .035 });
}

/* 指がボタンに触れる6回。既存のタップ音（下降スイープ）と正解のチャイムは残し、
...
for (const [index, tap] of TAPS.entries()) {
  ...
  /* フックは控えめ。場所の確定では伴奏に埋もれない強さにする。 */
  const chimeBoost = index === 0 ? .55 : tap.target === 'confirm' ? 1.3 : 1;
