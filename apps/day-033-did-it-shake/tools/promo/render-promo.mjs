...
if (!preview && !existsSync(audioPath)) {
  ...
  execFileSync(process.execPath, [audioScript, '--variant', 'a'], { stdio: 'inherit' });
}
...
const work = mkdtempSync(join(tmpdir(), 'day033-promo-'));
...
try {
  ...
  /* 発表は実APIを叩かない。撮るたびに発表が変わると、動画と投稿文が食い違う。
     返すのはテストと同じ固定応答（固定時計以前の発表だけをアプリが採用）。 */
  const announcements = await readFile(resolve(appDir, 'tests/fixtures/jma-list-2026-09-08.json'), 'utf8');
  await context.route('https://www.jma.go.jp/**', (route) => route.fulfill({
    status: 200, contentType: 'application/json; charset=utf-8', body: announcements
  }));
  // 差し替えたはずの先以外へ通信していないかを見張る（撮り終わりに報告する）
  const known = new Set([`127.0.0.1:${server.address().port}`, 'www.jma.go.jp']);
  ...
  if (preview) {
    ...
    execFileSync('ffmpeg', ['-y','-loglevel','error','-framerate','1','-i',join(work,'frame_%05d.jpg'),
      '-vf',`scale=360:640:flags=lanczos,tile=4x${rows}:padding=8:margin=8:color=f5f6f8`,'-frames:v','1','-q:v','2',contactPath], { stdio:['ignore','ignore','inherit'] });
  ...
  console.log(`想定外の外部通信: ${strangers.size ? [...strangers].join(', ') : 'なし（発表は固定応答）'}`);
} catch (error) {
