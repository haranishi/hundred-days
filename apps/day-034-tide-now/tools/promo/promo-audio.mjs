...
const SECTION = {
  ...
  curve: onBeat(sceneStart('S3')),
  station: onBeat(sceneStart('S4')),
  confirm: onBeat(T_CONFIRM_TAP),
  tables: onBeat(sceneStart('S5')),
...
for (let bar = 0; bar < BAR_COUNT; bar += 1) {
  ...
  if (leadOn) {
    ...
    const doubled = barStart >= SECTION.curve && barStart < SECTION.tables;
    for (const [offset, midi, length] of motif) {
      const time = beat(offset);
      if (time >= SECTION.promise) continue;
      addPiano({
        start: time, duration: BEAT * length + .35, midi,
        gain: (VARIANT === 'a' ? .13 : .15) * (barStart >= SECTION.tables ? .8 : 1), pan: -.1
      });
      if (doubled) {
        addPiano({
...
// 場面の変わり目に置く、ごく薄い持ち上げ
for (const at of [SECTION.answer, SECTION.curve, SECTION.tables]) {
