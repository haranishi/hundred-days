/* demo.mp4 の頭を切り落とす。
   録画は「読込中 → 地方選び → はじめる」から素直に撮っているが、
   1コマ目がそのままXのサムネになるので、文字画面から始めてはいけない
   （2026-08-19の本人指示）。地名が迫ってきて打鍵が見えている位置まで捨てる。

   使い方: node tools/trim-demo.mjs [切り落とす秒数]
   既定は1.85秒。振り付け（demo-scenario.mjs）を変えたら、切る位置も測り直すこと。 */
...
const head = Number(process.argv[2] ?? 1.85);
