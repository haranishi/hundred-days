/* プロモ動画の BGM。外部パッケージも外部音源も使わず、固定シードの純JSで合成する。
   ...
   既定（--variant a）は必ず同じフォルダへ promo-audio.wav を 35.000秒 / 48kHz / 16bit / stereo で書く。
...
const BAR = BEAT * 4;               // 2.6087秒（35秒 ≈ 13.4小節）
...
const SECTION = {
  answer: onBeat(sceneStart('S1')),   // 答えが出る（東京駅・351件）
  coins: onBeat(sceneStart('S2')),    // 内訳
  near: onBeat(sceneStart('S3')),     // 集まっている地点
  chart: onBeat(sceneStart('S4')),    // 時刻の帯
  confirm: onBeat(T_CONFIRM_TAP),     // 候補を押して答えが入れ替わる（いちばんの山）
  facts: onBeat(sceneStart('S5')),    // 場所を高知駅に変える
  caveat: onBeat(sceneStart('S6')),   // 出典と限界
