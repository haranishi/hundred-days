...
export const heatOf = shotHeat => Array.from({ length: COUNT }, (_, i) => Math.min(1, (shotHeat[i] || 0) / 6));
export const decayHeat = (shotHeat, dt) => shotHeat.map(v => v * Math.exp(-dt / 5));
...
export const outsmarted = (heat, lane) => heat[lane] < heat.reduce((a, b) => a + b, 0) / 12;
