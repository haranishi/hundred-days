# Day047 v2 実装契約（物理・敵・検証核＝Codex）
目的: DESIGN-v2.md §2〜§5・§8 の「1周目」を、境界 day047-v2-boundary.md のとおりに `lib/physics.js` `lib/entities.js` `lib/levels.js` `lib/autopilot.js` と自分のテストへ実装する。
...
検証: `cd <ローカルパス> && node --test day-047-namahage-night/tests/` を自分で実行。他Dayを壊さない（`node --test day-*/tests/`）。
