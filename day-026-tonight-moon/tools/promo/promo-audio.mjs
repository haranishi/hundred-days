...
function randomFactory(seed = 0x260902) {
  let value = seed >>> 0;
  return () => {
    value ^= value << 13;
    value ^= value >>> 17;
...
// 薄いパッド。わずかなデチューンと非常に遅い呼吸だけを持たせる。
const pads = [
  { frequency: 110 * 2 ** (-4 / 1200), gain: .058, pan: -.46, phase: .1 },
  { frequency: 110 * 2 ** (3 / 1200), gain: .052, pan: .42, phase: 1.7 },
  { frequency: 164.8138 * 2 ** (-2 / 1200), gain: .038, pan: .08, phase: 3.2 }
];
...
// 72 BPM の8分音符。固定シードで同じ音列を毎回作る。
const notes = [220, 261.6256, 293.6648, 329.6276, 391.9954, 440];
...
for (let step = 0, start = .8; start < 34.2; step += 1, start = .8 + step * eighth) {
  ...
  const gain = .052 + random() * .017;
  const base = { start, duration: 1.34, frequency, gain, pan, attack: .035, decay: .22,
    sustain: .46, release: .68, triangleMix: .44, phaseOffset: random() * TAU };
  addTone(base);
  // フィードバックディレイを2回だけ展開する。反響は左右をゆっくり渡る。
  addTone({ ...base, start: start + .39, gain: gain * .23, pan: -pan * .8, duration: 1.18 });
  addTone({ ...base, start: start + .78, gain: gain * .075, pan: pan * .55, duration: 1.02 });
}
...
// 帳のスタンプに、A5→E6の小さな2音チャイム。
for (const [start, frequency, pan] of [[T_STAMP, 880, -.18], [T_STAMP + .15, 1318.5102, .24]]) {
